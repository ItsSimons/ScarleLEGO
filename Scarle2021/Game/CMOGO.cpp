...
void CMOGO::Draw(DrawData* _DD)
{
	...
	m_model->Draw(_DD->m_pd3dImmediateContext, *_DD->m_states, //graphics device and CommonStates reuqired by model system
		...
		//this VERY weird construction creates a function on the fly to set up the render states correctly else the model system overrides them BADLY
		[&](){_DD->m_pd3dImmediateContext->RSSetState(s_pRasterState);} //<-Lambda
		);
